...
public class DroneControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GetComponent<Move_drone>().enabled = false;
    ...
    void Update()
    {
        ...
        //移動
        v = Input.GetAxis("L_S_Horizontal");
        h = Input.GetAxis("L_S_Vertical");
        ...
        //{
        if (Input.GetButton("Jump")) jumpflg = true;
        //}
        //if (h > 0.01f || v > 0.1f)
        //{
    ...
    void FixedUpdate()
    {
        ...
        if (jumpflg)
        {
            //rb.velocity = new Vector3(rb.velocity.x, jumpSpeed, rb.velocity.z);
            //rb.AddForce(transform.up * jumpSpeed);
            rb.velocity = new Vector3(v, jumpSpeed, h);
            isMoving = true;
            jumpflg = false;
            Debug.Log("jump");
        }
